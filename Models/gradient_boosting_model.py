...
class GasLevelGradientBoostingModel:
    ...
    de minas subterráneas usando Gradient Boosting, considerando factores operacionales como temperatura,
        ...
        for battery_range in battery_analysis.index:
            if battery_range is not None:  # Evitar problemas con NaN
                ...
                result[str(battery_range)] = {
                    # 'gas_mean': float(battery_analysis.loc[battery_range, ('nivel_gas_metano', 'mean')]),
                    # 'gas_std': float(battery_analysis.loc[battery_range, ('nivel_gas_metano', 'std')]),
    ...
    def analyze_temperature_impact(self):
        ...
        for temp_range in temp_analysis.index:
            if temp_range is not None:
                ...
                result[range_str] = {
                    # 'gas_mean': float(temp_analysis.loc[temp_range, ('nivel_gas_metano', 'mean')]),
                    # 'gas_std': float(temp_analysis.loc[temp_range, ('nivel_gas_metano', 'std')]),
                    'gas_mean': self.ppm_to_percentage(gas_mean_ppm),
                    'gas_std': self.ppm_to_percentage(gas_std_ppm),
    ...
    def train_model(self, force_retrain=False):
        """
        Entrena el modelo de Gradient Boosting para predecir niveles de gas, aplicando
